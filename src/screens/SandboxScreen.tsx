...
:root {
  ...
  return (
    <div className="p-6 pt-12 animate-fade-in pb-32 h-full flex flex-col gap-6 overflow-y-auto no-scrollbar">
      {/* Header */}
      <header className="neu-flat rounded-[28px] p-7 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 border border-[var(--color-neu-shadow-light)]/40">
        <div className="flex items-center gap-4">
          <SipCodeEmblem size="md" withGlow />
          <div>
        ...
        <div className="neu-pressed px-4 py-2 rounded-[18px] flex items-center gap-2.5 shrink-0">
          <div 
            className="w-3.5 h-3.5 rounded-full border border-black/20"
            style={{ backgroundColor: activeTheme.colors.accent }}
          />
          <div>
            <span className="text-[9px] uppercase tracking-widest font-bold text-[var(--color-neu-text-light)] block leading-none">
              Active Theme
            </span>
            <span className="text-xs font-bold text-[var(--color-neu-text)] font-sans">
              {activeTheme.name}
            </span>
          </div>
        </div>
