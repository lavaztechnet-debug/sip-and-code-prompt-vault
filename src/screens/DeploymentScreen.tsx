...
`;
  ...
  return (
    <div className="p-6 pt-12 animate-fade-in pb-32 h-full flex flex-col gap-6">
      {/* Header */}
      <header className="neu-flat rounded-[28px] p-6 flex items-center justify-between border border-[var(--color-neu-shadow-light)]/40">
        <div className="flex items-center gap-3.5">
          <SipCodeEmblem size="sm" withGlow />
          <div>
            <div className="flex items-center gap-1.5">
              <span className="text-[10px] uppercase tracking-widest font-bold text-[var(--color-neu-accent)]">On-Device Subsystem</span>
            </div>
            <h1 className="text-lg font-serif italic text-[var(--color-neu-text)] mt-0.5">Termux &amp; LLM Deployment</h1>
            <p className="text-[10px] text-[var(--color-neu-text-light)]">Zero-Gradle Pipelines, Local Models &amp; Multi-Format Vault Exporters</p>
          </div>
        </div>
        <button
          onClick={() => setShowTermuxModal(true)}
          className="neu-button p-3 rounded-[16px] text-[var(--color-neu-accent)] cursor-pointer shrink-0"
